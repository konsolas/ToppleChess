...
movesort_t::movesort_t(GenMode mode, search_t &search, move_t hash_move, int ply) :
        mode(mode), search(search), hash_move(hash_move), ply(ply), gen(movegen_t(search.board)) {}
...
move_t movesort_t::next(GenStage &stage, int &score) {
    ...
    switch (stage) {
        case GEN_NONE:
            stage = GEN_HASH;
            if (search.board.is_pseudo_legal(hash_move)) {
                score = INF;
                return hash_move;
            }
        case GEN_HASH:
            ...
            for (int i = 0; i < capt_buf_size; i++) {
                int see_score = search.board.see(capt_buf[i]);
                capt_scores[i] = see_score >= 0 ? (see_score +
                                  (search.board.record[search.board.now].next_move ?
        ...
        case GEN_GOOD_CAPT:
            ...
            for (int i = 0; i < main_buf_size; i++) {
                if (search.killer_heur.primary(ply) == main_buf[i]) {
                    main_scores[i] = KILLER_BASE + 3;
                } else if (search.killer_heur.secondary(ply) == main_buf[i]) {
                    main_scores[i] = KILLER_BASE + 2;
                } else if (ply >= 2 && (search.killer_heur.primary(ply - 2) == main_buf[i])) {
                    main_scores[i] = KILLER_BASE + 1;
                } else if (ply >= 2 && (search.killer_heur.secondary(ply - 2) == main_buf[i])) {
                    main_scores[i] = KILLER_BASE;
                } else {
                    main_scores[i] = search.history_heur.get(main_buf[i]);
                }
            }
